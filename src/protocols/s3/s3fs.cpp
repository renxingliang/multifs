...
static int my_fuse_opt_proc(const char* arg);
...
int s3fs_getattr(const char* _path, struct stat* stbuf)
{
	WTF8_ENCODE(path)
		int result;
...
int s3fs_unlink(const char* _path)
{
	WTF8_ENCODE(path)
		int result;
...
int s3fs_open(const char* _path, struct fuse_file_info* fi, mode_t mode)
{
	WTF8_ENCODE(path)
		int result;
	struct stat st;
...
int s3fs_flush(const char* _path, struct fuse_file_info* fi)
{
	WTF8_ENCODE(path)
		int result;
...
static int my_fuse_opt_proc(const char* arg)
{
	...
	if (0 == STR2NCMP(arg, "umask=")) {
		...
		is_s3fs_umask = true;
		return 0; // continue for fuse option
	}
